...
export function toolSchemas(ctx: ToolContext) {
  ...
  return [
    ...
    fn("drug_price", "Look up LIVE cash/coupon prices for a medication from public pharmacy pages (GoodRx), returning real per-pharmacy dollar amounts. Use when the user asks what a drug costs, where it's cheapest, or how to save on a prescription. Prices are cash/coupon estimates, not insurance.", { drug: { type: "string", description: "medication name, e.g. 'atorvastatin' or 'metformin'" } }, ["drug"]),
